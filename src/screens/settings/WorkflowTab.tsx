...
import { GripVertical, Plus, Play, Trash2, X } from 'lucide-react-native';
import { useSettingsPageColors } from '../../theme/colors';
import { useWorkflowStore } from '../../stores/workflows';
import { useSettingsStore } from '../../stores/settings';
import type { WorkflowNodeType } from '../../types/workflow';
import { getWorkflowToolDescription, WORKFLOW_TOOL_NAMES } from '../../services/workflowToolCatalog';
...
export function WorkflowTab({ showToast, keyboardBottomInset }: Props) {
  ...
  const availableTools = useMemo(() => {
    const mcpDefinitions = mcpRemoteTool.getDefinitions({ memoryVault: false, webSearch: false, mcpTools: mcpToolConfig });
    return [
      ...WORKFLOW_TOOL_NAMES.map((name) => ({ name, description: getWorkflowToolDescription(name) })),
      ...mcpDefinitions.map((tool) => ({
        name: tool.function.name,
        description: getWorkflowToolDescription(tool.function.name, tool.function.description),
      })),
    ];
  }, [mcpToolConfig]);
  ...
  return <View style={styles.root}>
    ...
    <ScrollView scrollEnabled={canvasScrollEnabled} style={styles.canvasScroll} contentContainerStyle={{ minHeight: Math.max(700, ...workflow.nodes.map((node) => node.y + 190)), paddingBottom: keyboardBottomInset }}><View style={styles.canvas}>
      ...
      {workflow.nodes.map((node) => <CanvasNode key={node.id} node={node} selectedLink={linkFrom === node.id} onMove={(x: number, y: number) => actions.updateNode(workflow.id, node.id, { x, y })} onDragStart={() => setCanvasScrollEnabled(false)} onDragEnd={() => setCanvasScrollEnabled(true)} onDelete={() => actions.deleteNode(workflow.id, node.id)} onConnect={() => { if (linkFrom && linkFrom !== node.id) { actions.addEdge(workflow.id, linkFrom, node.id); setLinkFrom(null); } else setLinkFrom(linkFrom === node.id ? null : node.id); }} onConfig={(config: any) => actions.updateNode(workflow.id, node.id, { config })} onPickTools={() => { setToolPickerAgentId(node.id); setToolPicker(true); }} styles={styles} colors={colors}/>) }
    </View></ScrollView>
    <Modal visible={toolPicker} transparent animationType="fade" onRequestClose={() => setToolPicker(false)}><View style={styles.modalBackdrop}><View style={styles.toolModal}><View style={styles.modalHeader}><Text style={styles.modalTitle}>选择此 AI 可使用的工具</Text><Pressable onPress={() => setToolPicker(false)}><X color={colors.text}/></Pressable></View><ScrollView>{availableTools.map((tool) => { const agent = workflow.nodes.find((node) => node.id === toolPickerAgentId); const selected = Array.isArray(agent?.config.toolNames) && agent.config.toolNames.includes(tool.name); return <Pressable key={tool.name} style={[styles.toolRow, selected && styles.toolRowSelected]} onPress={() => { if (!agent) return; const current = Array.isArray(agent.config.toolNames) ? agent.config.toolNames : []; const toolNames = selected ? current.filter((item: string) => item !== tool.name) : [...current, tool.name]; actions.updateNode(workflow.id, agent.id, { config: { ...agent.config, toolNames } }); }}><View style={styles.toolTitleRow}><Text style={[styles.toolCheck, selected && { color: colors.primary }]}>{selected ? '✓' : '＋'}</Text><View style={styles.toolText}><Text style={[styles.toolLabel, selected && { color: colors.primary }]}>{getToolLabel(tool.name)}</Text><Text style={styles.toolDescription}>{tool.description}</Text><Text style={styles.toolName}>{tool.name}</Text></View></View></Pressable>; })}</ScrollView></View></View></Modal>
  </View>;
}

function CanvasNode({ node, selectedLink, onMove, onDragStart, onDragEnd, onDelete, onConnect, onConfig, onPickTools, styles, colors }: any) {
  ...
  const pan = useMemo(() => PanResponder.create({
    ...
    onPanResponderGrant: () => {
      origin.current = { x: nodeRef.current.x, y: nodeRef.current.y };
      onDragStartRef.current?.();
    },
    onPanResponderMove: (_, gesture) => onMoveRef.current(Math.max(0, origin.current.x + gesture.dx), Math.max(0, origin.current.y + gesture.dy)),
  ...
  return <View style={[styles.node, { left: node.x, top: node.y }, selectedLink && styles.nodeLinking]}>
    <View style={styles.nodeHeader}><View style={styles.nodeDragHandle} {...pan.panHandlers}><GripVertical size={15} color={colors.textTertiary}/><Text numberOfLines={1} style={styles.nodeTitle}>{node.title}</Text></View><Pressable hitSlop={8} onPress={onDelete}><X size={15} color={colors.textSecondary}/></Pressable></View>
